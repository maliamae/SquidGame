...
[RequireComponent(typeof(WaterTrigggerHandler))]
...
public class InteractableWater : MonoBehaviour
{
    ...
    [Range(2, 500)] public int NumOfXVerts = 70;
    public float Width = 10f;
    public float Height = 4f;
    public Material WaterMaterial;
    private const int NUM_OF_Y_VERTS = 2;
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 1; i < _waterPoints.Count -1; i++)
        {
            ...
            _vertices[_topVertsIndex[i]].y = point.pos;
    ...
    public void Splash(Collider2D collision, float force)
    {
        ...
        {
            Vector2 vertexWorldPos = transform.TransformPoint(_vertices[_topVertsIndex[i]]);
    ...
    public void ResetEdgeCollider()
    {
        ...
        Vector2 firstPoint = new Vector2(_vertices[_topVertsIndex[0]].x, _vertices[_topVertsIndex[0]].y);
        newPoints[0] = firstPoint;

        Vector2 secondPoint = new Vector2(_vertices[_topVertsIndex[_topVertsIndex.Length - 1]].x, _vertices[_topVertsIndex[_topVertsIndex.Length - 1]].y);
    ...
    public void GenerateMesh()
    {
        ...
        //add vertices
        _vertices = new Vector3[NumOfXVerts * NUM_OF_Y_VERTS];
        _topVertsIndex = new int[NumOfXVerts];
        for (int y = 0; y < NUM_OF_Y_VERTS; y++)
        {
            for (int x = 0; x < NumOfXVerts; x++)
            {
                float xPos = (x / (float)(NumOfXVerts - 1) * Width - Width/2);
                float yPos = (y / (float)(NUM_OF_Y_VERTS - 1) * Height - Height/2);
                _vertices[y * NumOfXVerts + x] = new Vector3(xPos, yPos, 0f);

                if (y == NUM_OF_Y_VERTS - 1)
                {
                    _topVertsIndex[x] = y * NumOfXVerts + x;
                }
            }
        }

        //construct triangles 
        int[] triangles = new int[(NumOfXVerts - 1) * (NUM_OF_Y_VERTS - 1) * 6];
        int index = 0;

        for (int y = 0; y < NUM_OF_Y_VERTS - 1; y++)
        {
            for (int x = 0; x < NumOfXVerts - 1; x++)
            {
                int bottomLeft = y * NumOfXVerts + x;
                int bottomRight = bottomLeft + 1;
                int topLeft = bottomLeft + NumOfXVerts;
    ...
    private void CreateWaterPoints()
    {
        ...
        for (int i = 0; i < _topVertsIndex.Length; i++)
        {
            _waterPoints.Add(new WaterPoint
            {
                pos = _vertices[_topVertsIndex[i]].y,
                targetHeight = _vertices[_topVertsIndex[i]].y,
            });
        }
    }
}
